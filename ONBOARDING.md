...
**IV Rank (true, 52-week):** Laura's "don't buy LEAPS when IV is inflated" needs *implied* vol ranked over ~1 year — E*Trade's definition: `(todayIV − 52wk_low) / (52wk_high − 52wk_low) × 100`. Neither TwelveData (price-only) nor MarketData.app (snapshot IV, no history) hands us a ready IV Rank, so the **worker** builds one: it samples ~30-day ATM IV weekly from **MarketData.app's historical option chains** (`?date=…&dte=30`, the `iv` field), caches the series in the D1 `iv_history` table, and ranks today's ATM IV within the trailing 52 weeks. Requires **≥20 samples** to publish (`IV_MIN_SAMPLES`); backfills up to **4 missing weeks per request** (`IV_BACKFILL_MAX`, bounded so it never spikes the API), so it seeds over the first several refreshes, then extends for free (today's sample comes from the CSP chain we already fetch). Everything is best-effort in try/catch → falls back to the old **cross-strike proxy** on any failure. The worker returns `ivRankSource` (`'true'` | `'proxy'`) + `ivSamples`; the frontend labels it **"· 52-wk"** vs **"· est · seeding N/20"** and analyzer pages (`resolveIvRank`) prefer the card's true value for watchlist tickers. **Auto-seeding:** the client calls **`/api/iv-seed?ticker=X`** (`handleIvSeed`) in the background — throttled 2 at a time — for every watchlist ticker not yet on the true rank, on startup and whenever tickers change (`autoSeedIvRank`), so users never wait through manual refreshes. That endpoint fully seeds ONE ticker (`backfillIvHistory(…, 52)`) in a single request (bounded subrequest budget), then returns its true rank. `/api/scores` also does a small bounded backfill (`IV_BACKFILL_MAX=10`) as a fallback. Dates anchor to a stable 52-Friday grid so backfill settles once seeded; failed fetches are left to retry, genuine no-data days get a null marker so they're not retried forever. *(The old "IV Rank" was NOT E*Trade's — it was where the median IV sat between a single chain's min/max strikes, i.e. a skew-position proxy, and never matched a broker's IV Rank.)*
