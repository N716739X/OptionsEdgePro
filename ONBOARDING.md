...
| Synthetic Long | MR ≤ −2 | same-strike ATM call+put (Course 301 SL — sell ATM put + buy ATM call), furthest LEAPS; **3 scored criteria** — DTE ≥540, **net debit ≤8%** (course SLs run ~5.5–7.5%: CPER $2/$36=5.6%, MRVL $6/$80=7.5%), both spreads ≤10%. **IV Rank is INFORMATIONAL, not scored** (`c2 = null`, `ivInfoRow`): per Laura's message the SL is IV-neutral — the expensive put you sell funds the expensive call you buy (put-call parity → same-strike net debit is IV-independent), so high IV is NOT a disqualifier for an SL (it's the structure she *uses* in high IV; IV matters for straight LEAPs / MSLs, which is why the MSL captures it via debit/risk). IV Rank still displays for context. Breakeven = strike+debit; max loss = strike+debit **to zero**; margin = ½·strike·100 + debit. Prominent **margin-call risk** caution (leveraged margin play, not defined-risk). **Find Best / Grade Mine toggle** (mirrors MSL): Grade Mine takes your Buy Call + Sell Put strikes + expiry, grades on the same 3 criteria, and shows a side-by-side vs the app's same-strike ATM pick (`synthComputeGrade` / `synthBreakeven` / `synthAtmIdx` / `gradeSynth`). Different-strike entries (e.g. CRCL 50c/80p) are graded honestly but flagged as a **risk reversal**, not a pure synthetic. |
...
**IV Rank (live, Barchart-seeded):** Laura reads *"Current IV%"* (IV percentile / rank) to decide LEAPS-vs-stock — buy leverage when IV is **low** in its own 1-yr range, buy stock when it's high. IV Rank = `(todayIV − 52wk_low) / (52wk_high − 52wk_low) × 100` (Barchart's/E*Trade's formula), which only needs the **52-week high & low**. Neither TwelveData (price-only) nor MarketData.app (**no historical IV** — confirmed: their historical chains return the `iv` array empty) can supply the year of IV needed to derive high/low ourselves, so we **seed the high/low per ticker from barchart.com** (`IV_SEED` map in the worker, user-provided from Barchart's Options Overview) and compute the rank **live** from today's ~30-day ATM IV (`atmIvFromChain` on the ~30-45 DTE put chain — matches Barchart's "nearest monthly ≥30d ATM" IV). **Forward-accumulation (self-maintaining):** `computeIvRank` records today's live IV into `iv_history` once per day and ranks against the trailing-365-day high/low the app has collected. While young it uses the **seed** (widened by any collected extreme) — `ivRankSource: 'seed'`; once it has ~a year of its own data (**≥60 samples spanning ≥300 days**) it switches to its **own** trailing range — `ivRankSource: 'accum'` — which ages out stale old extremes (auto-narrows, not just widens) and **retires the seed**, so it becomes fully self-maintaining with no manual refresh. Worker sets `synthCh.c2 = rank ≤ 50` (ideal ≤30) and returns `ivRankSource` (`'seed'` | `'accum'` | `'proxy'`); the frontend (`resolveIvRank`/`ivRankLabel`) shows a clean **"IVR N"** for `seed`/`accum` tickers, **"· est"** for any ticker on the proxy fallback. To reseed or add a ticker, edit `IV_SEED` with its Barchart IV High/Low. *(History: earlier attempts to backfill a true 52-wk series from MarketData's historical chains failed because MarketData doesn't serve historical IV; that dead code — `ivRankState`/`backfillIvHistory`/`/api/iv-seed`/client `autoSeedIvRank` — is retired. The original "IV Rank" was neither rank nor percentile — it was a skew-position proxy that never matched a broker.)* **Note:** Barchart also shows IV **Percentile** (Laura's literal metric, "% of days below current IV"); that needs the full distribution, so it's a possible future add via forward-accumulation — IV Rank is the live, high/low-only proxy we ship now.

**Watchlist:** IA11 minus SATS → `['TSLA','NVDA','AMD','MRVL','PLTR','ALAB','AVGO','MU','GOOG']` (worker `IA11_TICKERS`, frontend `IA11`/`DEFAULT_TICKERS`); SATS is also filtered out of any saved `owp-tickers` on load. Trader tier adds custom names (GOLD/SLVR/CRCL etc.).
