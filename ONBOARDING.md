...
- Put-spread credit **≥42%** of width (target 50%); net debit **≤40%** of price (ideal ≤33%); LEAPS ≥365 DTE (prefer ≥540).
...
- **3× test (MM60, warning-only):** price needed for a 3× = **call strike + 3 × risk**. The analyzer shows the target and the % move required, and flags a **fail** when that target is above the **52-week high** (proxy for the all-time high — TwelveData gives no true ATH, so it's labeled). Not a hard scored gate because the ATH proxy is imperfect.
...
- **Put spread = buffer-first (Laura MM56).** The short put is NOT at-the-money — it's anchored as **far OTM as possible while the spread still pays ~50% credit** (42% hard floor). Selection (`runMsl` / `runMslPhase3` / `mslPickPutSpread` / Worker `scoreMslChains` — **four replicas, keep in sync**): scan short-put strikes below spot × lower long puts with width in **[18%, 30%] of price** (Laura's ~20% baseline, widened up to ~30% "to bring in a little more credit"); Pass A (credit ≥50%): buffer is a "get-enough-then-stop" criterion, liquidity is "more-is-better" for a LEAPS you roll repeatedly — so among ≥50% spreads that clear **Laura's ~20% baseline buffer**, take the **highest short-put OI** (tiebreak deeper buffer, then credit→50%); only if nothing reaches the 20% baseline (low-IV name) fall back to the **furthest-OTM** spread. (MRVL: picks 160 @ 25% OTM / 737 OI over 150 @ 30% / 129 OI — both deep buffers, the liquid one wins.) Pass B (nothing hits 50%) collects all spreads ≥42%, treats credits **within 2.5 percentage points of the best as a tie**, and breaks toward **liquidity (short-put OI) then buffer (further OTM)** — this stops the old strict-max-credit rule from chasing a fractional-% edge onto a thin strike (the GOOG 345/268-OI-over-340/927-OI mis-pick); Pass C = closest-to-50%. This self-adjusts: ~20% OTM in high IV (SPCX @ ~$150 → sell 120 / buy 80, 40 wide, 50% credit, 20% buffer), near-ATM in low IV (NVDA). Buffer (short-put %OTM) is shown as a headline.
