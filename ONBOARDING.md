...
| Synthetic Long | MR ≤ −2 | same-strike ATM call+put (Course 301 SL — sell ATM put + buy ATM call), furthest LEAPS; **4 scored criteria** — IV Rank ≤50 (not inflated, `c2`), DTE ≥540, **net debit ≤8%** (course SLs run ~5.5–7.5%: CPER $2/$36=5.6%, MRVL $6/$80=7.5%), both spreads ≤10%. Breakeven = strike+debit; max loss = strike+debit **to zero**; margin = ½·strike·100 + debit. Prominent **margin-call risk** caution (leveraged margin play, not defined-risk). **Find Best / Grade Mine toggle** (mirrors MSL): Grade Mine takes your Buy Call + Sell Put strikes + expiry, grades them on the same 4 criteria, and shows a side-by-side vs the app's same-strike ATM pick (`synthComputeGrade` / `synthBreakeven` / `synthAtmIdx` / `gradeSynth`). Different-strike entries (e.g. CRCL 50c/80p) are graded honestly but flagged as a **risk reversal**, not a pure synthetic. *(Fixed alongside: `runSynth` graded net-debit at ≤5% while labeling it "≤8%" — now ≤8%, matching the dashboard/worker.)* |
